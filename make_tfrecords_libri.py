...
def encoder_proc(noisy_filename, clean_path, out_file, wav_canvas_size):
    """ Read and slice the wav and noisy files and write to TFRecords.
        out_file: TFRecordWriter.
    """
    ppath, noisy_fullname = os.path.split(noisy_filename)
    
    wav_filename = os.path.join(clean_path, "_".join(noisy_fullname.split('_')[:-2])+'.wav')
...
def main(opts):
    ...
    with open(opts.cfg) as cfh:
        ...
        for dset_i, (dset, dset_desc) in enumerate(cfg_desc.items()):
            ...
            noisy_files = [os.path.join(noisy_dir, wav) for wav in
                           os.listdir(noisy_dir) if wav.endswith('.wav')]
            nfiles = len(wav_files)
            nnfiles = len(noisy_files)
            assert nfiles == nnfiles
            for m, noisy_file in enumerate(noisy_files):
                ...
                sys.stdout.flush()
                encoder_proc(noisy_file, wav_dir, out_file, 2 ** 14)
        out_file.close()
