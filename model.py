...
class SEGAN(Model):
    ...
    def build_model_single_gpu(self, gpu_idx):
        ...
        G, z, h  = self.generator(noisybatch, is_ref=False, spk=None,
                               do_prelu=do_prelu)
        self.Gs.append(G)
        self.zs.append(z)
        self.h.append(h)
    ...
    def clean(self, x):
        ...
        for beg_i in range(0, x.shape[0], self.canvas_size):
            ...
            canvas_w = self.sess.run(self.Gs[0],
                                     feed_dict=fdict)[0]
            h_latent = self.sess.run(self.h[0],
                                     feed_dict=fdict)[0]
            letent_total.append(h_latent)
        ...
        # deemphasize
        letent_total = np.reshape(letent_total, [-1, 8192])
        letent_total = letent_total[[0],:]
        f = open(os.path.join('.', 'h.txt'), 'ab')
        np.savetxt(f, letent_total)
        f.close()
